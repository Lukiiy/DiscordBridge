...
public class Console implements CommandPlate {
    ...
    public void interaction(CommandInteraction i) {
        ...
        String command = cmd;
        Bukkit.getServer().getScheduler().scheduleAsyncDelayedTask(DCBridge.getInstance(), () -> DCBridge.consoleCommand(command));
        DCBridge.log(DCBridge.configString("format.dcPrefix") + " " + member.getEffectiveName() + " executed \"/" + cmd + "\" as console.");
